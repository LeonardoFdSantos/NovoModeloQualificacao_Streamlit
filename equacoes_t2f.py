...
# Helpers Visuais
# -------------------------
def create_textbox(title_str, content_mobject, color=WHITE, bg_opacity=0.85):
    """
    Cria uma caixa com título e conteúdo, com fundo escuro para legibilidade.
    """
    title = Text(title_str, font_size=32, color=color, weight=BOLD)
    
    # Organiza título e conteúdo verticalmente
    content_group = VGroup(title, content_mobject).arrange(DOWN, buff=0.3, aligned_edge=LEFT)
    
    # Cria o fundo (backdrop)
...
def clip_time(t: float, ymin=0.0, ymax=10.0):
    # Proteção contra NaN e Infinito
    if t != t or t in (float("inf"), float("-inf")):
        return ymax
    return max(ymin, min(ymax, t))
...
class DerivacaoRMS_Tese(Scene):
    def construct(self):
        ...
        eq_raw = MathTex(
            ...
            r"\frac{1}{N \Delta t}",     # Denominador
            r"\sum_{n=0}^{N-1} i^2[n]",  # Somatório
            r"\Delta t",                 # O dt que veio da integral
            r"}"
        ).scale(1.2)
        
        eq_raw.set_color_by_tex("N", c_amostras)
        eq_raw.set_color_by_tex("Delta", c_tempo)
        ...
        self.play(
            ...
            FadeOut(rules_group),
            FadeOut(txt_sub),
            Write(eq_raw) # Usamos Write aqui para ficar limpo, pois os layouts eram muito diferentes
        )
        self.wait(1)
        ...
        corte_baixo = Line(start=eq_raw[3].get_corner(DL), end=eq_raw[3].get_corner(UR), color=RED, stroke_width=3).scale(0.3).shift(RIGHT*0.15 + DOWN*0.15)
        corte_cima = Line(start=eq_raw[5].get_corner(DL), end=eq_raw[5].get_corner(UR), color=RED, stroke_width=3)
        
        self.play(Create(corte_baixo), Create(corte_cima))
        self.wait(0.5)

        # Equação Limpa (RMS Global)
        eq_global = MathTex(
            r"I_{\mathrm{RMS}}", r"\approx", r"\sqrt{", 
            r"\frac{1}{N}", 
...
class Clarke_Tese(Scene):
    def construct(self):
        ...
        # Tamanhos de Fonte Padronizados (Levemente reduzidos para evitar cortes)
        FS_TITLE = 34
        FS_SUBTITLE = 26
        FS_TEXT = 20
        FS_MATH = 36
        
        # Margens de segurança (Buffs) - AUMENTADAS para evitar cortes laterais
        EDGE_BUFF_SIDE = 1.5  # Margem lateral generosa
        EDGE_BUFF_TOP = 0.8   # Margem superior
        
        # Título Principal
        main_title = Tex(r"Transformação de Clarke: $\mathbf{abc} \rightarrow \boldsymbol{\alpha\beta}$", font_size=FS_TITLE)
        main_title.to_edge(UP, buff=EDGE_BUFF_TOP)
        self.add(main_title)

        # =================================================================
        # ETAPA 1: Geometria Vetorial (Centro da Tela)
        # =================================================================
        
        # Centro do diagrama deslocado para baixo para não brigar com o título
        origin = DOWN * 0.8
        ...
        vec_b = Arrow(origin, origin + 2.5*np.array([np.cos(2*np.pi/3), np.sin(2*np.pi/3), 0]), color=c_b, buff=0)
        lbl_b = MathTex("i_b", color=c_b).next_to(vec_b.get_end(), UP+LEFT, buff=0.1)
        
        vec_c = Arrow(origin, origin + 2.5*np.array([np.cos(-2*np.pi/3), np.sin(-2*np.pi/3), 0]), color=c_c, buff=0)
        ...
        # =================================================================
        # ETAPA 2: Transição e Matriz (Dividindo a tela com margens seguras)
        # =================================================================
        
        diagrama_full = VGroup(group_abc, group_clarke)
        
        # Move para a esquerda com MUITO mais margem (buff=EDGE_BUFF_SIDE)
        self.play(
            diagrama_full.animate.scale(0.5).to_edge(LEFT, buff=EDGE_BUFF_SIDE).shift(DOWN*0.5)
        )

        txt_projecao = Text("Projeção nos eixos ortogonais", font_size=FS_TEXT, color=GRAY)
        # Centraliza o texto em relação ao diagrama reduzido
        ...
        # Posiciona na direita com margem segura
        ...
        txt_row1 = Text("Eixo Alpha (horizontal)", font_size=FS_TEXT, color=c_alpha)
        txt_row1.next_to(arrow_row1, UP, buff=0.1)
        ...
        # =================================================================
        # ETAPA 3: Dedução Algébrica (Centralizada)
        # =================================================================
        ...
        # --- 3.1 Derivando Alpha ---
        # Cabeçalho com margem lateral segura
        header_alpha = Text("1. Derivando Alpha", font_size=FS_SUBTITLE, color=c_alpha)
        header_alpha.to_edge(LEFT, buff=EDGE_BUFF_SIDE).shift(UP*1.5)
        self.play(Write(header_alpha))

        # Equações centralizadas
        eq_a1 = MathTex(
        ...
        eq_a2 = MathTex(
            r"i_\alpha = \frac{2i_a - i_b - i_c}{3}",
            font_size=FS_MATH+10, color=c_alpha
        )
        self.play(ReplacementTransform(eq_a1, eq_a2))
        self.wait(1)
        
        # Move para área de espera com margem segura
        eq_a_stored = eq_a2.copy().scale(0.7)
        # Posiciona abaixo do título e alinhado à esquerda com margem
        ...
        # --- 3.2 Derivando Beta ---
        header_beta = Text("2. Derivando Beta", font_size=FS_SUBTITLE, color=c_beta)
        header_beta.to_edge(LEFT, buff=EDGE_BUFF_SIDE).shift(UP*0)
        self.play(Write(header_beta))

        # Passo 1: Equação expandida
        # Índice visual: 
        # eq_b1[2] é o termo "2/3"
        # eq_b1[4] é o termo "\sqrt{3}/2"
        eq_b1 = MathTex(
            r"i_\beta", r"=", 
            r"\frac{2}{3}", r"\cdot", r"\frac{\sqrt{3}}{2}", 
            r"(i_b - i_c)",
            font_size=FS_MATH+5
        )
        ...
        # Passo 2: O Corte CORRETO (2 com 2)
        # Pegamos especificamente o numerador da primeira fração e denominador da segunda
        num_2 = eq_b1[2][0] # O número '2' superior
        den_2 = eq_b1[4][4] # O número '2' inferior (dependendo da fonte pode ser índice 2 ou 3, geralmente é o último char do termo)
        
        slash_1 = Line(num_2.get_corner(DL), num_2.get_corner(UR), color=c_corte, stroke_width=4).scale(1.2)
        slash_2 = Line(den_2.get_corner(DL), den_2.get_corner(UR), color=c_corte, stroke_width=4).scale(1.2)
        
        self.play(ShowPassingFlash(slash_1.copy().set_color(WHITE)), Create(slash_1))
        self.play(ShowPassingFlash(slash_2.copy().set_color(WHITE)), Create(slash_2))
        self.wait(0.5)

        # Passo 3: Resultado Intermediário (sqrt(3) / 3)
        # Isso prova que não cortamos a raiz, apenas os 2
        eq_b_mid = MathTex(
            r"i_\beta", r"=", 
            r"\frac{\sqrt{3}}{3}", 
            r"(i_b - i_c)",
        ...
        self.play(
            FadeOut(slash_1), FadeOut(slash_2),
            TransformMatchingTex(eq_b1, eq_b_mid)
        )
        self.wait(1)

        # Passo 4: Simplificação final (Racionalização Inversa)
        # Explica visualmente que sqrt(3)/3 vira 1/sqrt(3)
        eq_b2 = MathTex(
            r"i_\beta = \frac{i_b - i_c}{\sqrt{3}}",
            font_size=FS_MATH+10, color=c_beta
        )
        ...
        # =================================================================
        # ETAPA 4: Resumo Final (Centralizado)
        # =================================================================
        ...
        # Agrupa e centraliza tudo na tela
        grupo_formulas = VGroup(final_alpha, final_beta).arrange(DOWN, buff=0.8)
        
        self.play(
            final_alpha.animate.scale(1.4), # Restaura tamanho original
            grupo_formulas.animate.move_to(ORIGIN) # Centraliza na tela
        )

        box = SurroundingRectangle(grupo_formulas, color=WHITE, buff=0.5, corner_radius=0.2)
        lbl_final = Text("Fórmulas Otimizadas para MCU/DSP", font_size=FS_TEXT).next_to(box, DOWN)
...
class EqSeq_Tese(Scene):
    def construct(self):
        ...
        defs = VGroup(
            ...
            MathTex(r"1 + a + a^2 = 0", font_size=FS_MATH, color=YELLOW)
...
class EqSeqInst_Tese(Scene):
    def construct(self):
        ...
        # Cores
        c_sig = YELLOW     # Sinal Original (Analítico)
        ...
        txt_cancel = Text("As velocidades angulares se cancelam!", font_size=20, color=YELLOW).next_to(origin_2, DOWN, buff=2)
...
class EqTCC_Tese(Scene):
    ...
    def construct(self):
        ...
        explanation = VGroup(
            ...
            Text("• Se M > 1: O relé começa a contar tempo.", font_size=24, color=YELLOW)
        ...
        demo_txt = Text("Exemplo: Very Inverse (k=13.5, α=1, TMS=1)", font_size=24, color=YELLOW)
        ...
        demo_txt_ieee = Text("Exemplo: Moderately Inv (A=0.0515, B=0.114, p=0.02, TD=1)", font_size=24, color=YELLOW)
        ...
        offset_desc = Text("O termo '+ B' garante um tempo mínimo de atuação", font_size=24, color=YELLOW).next_to(eq_ieee, DOWN, buff=0.3)
        self.play(Write(offset_desc))
        self.play(Indicate(eq_ieee[0][14:], color=YELLOW)) # Indica a parte "+ 0.114 )"
        self.wait(2)
...
class EqTCCNum_Tese(Scene):
    def construct(self):
        ...
        eq_general.set_color_by_tex("t(I)", YELLOW)
        ...
        eq_general.set_color_by_tex("I", YELLOW) 
        # O "I_p" contém "I", então garantimos que ele fique amarelo também
        eq_general.set_color_by_tex("I_p", YELLOW)
        ...
        params = VGroup(
            ...
            MathTex(r"I = 500\text{A}, \quad I_p = 100\text{A}", color=YELLOW)
        ...
        # 1. Variável t
        eq_numeric.set_color_by_tex("t", YELLOW)
        ...
        eq_numeric.set_color_by_tex("500", YELLOW) # I
        eq_numeric.set_color_by_tex("100", YELLOW) # Ip
        ...
        # 4. CONCLUSÃO
        # ==========================================
        final_box = SurroundingRectangle(eq_numeric, color=YELLOW, buff=0.2)
        result_text = MathTex(r"\approx 2.97\text{ s}", color=YELLOW).next_to(eq_numeric, RIGHT)
