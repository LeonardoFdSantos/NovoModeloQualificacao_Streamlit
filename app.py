...
def calculate_thd(freq_array, amp_array, fund_freq=60, max_freq=2000):
    if freq_array.size == 0: return 0
    amp_h1 = get_amplitude_at_order(freq_array, amp_array, 1, fund_freq)
    if amp_h1 == 0: return 0
    
    sum_squares = 0
    for order in range(2, int(max_freq/fund_freq)):
        amp_hn = get_amplitude_at_order(freq_array, amp_array, order, fund_freq)
...
def processar_arquivos(uploaded_files):
    ...
    for p in PONTOS_BASE:
        for var, label in [('I', 'Corrente'), ('V', 'Tensão')]:
            ...
            for fase in FASES:
                fig_t_key = f'{key_root}_T_{fase}'; line_figs[fig_t_key] = go.Figure(layout=go.Layout(title=f'Tempo: {var}_{p} (Fase {fase})', xaxis_title='Tempo (s)', yaxis_title='Amp.')); aplicar_estilo_grafico(line_figs[fig_t_key])
                fig_f_key = f'{key_root}_F_{fase}'; line_figs[fig_f_key] = go.Figure(layout=go.Layout(title=f'FFT: {var}_{p} (Fase {fase})', xaxis_title='Frequência (Hz)', yaxis_title='Amplitude (dB)', xaxis=dict(range=[0, F_MAX_ANALISE/2], autorange=False))); aplicar_estilo_grafico(line_figs[fig_f_key])
    ...
    for i, file in enumerate(uploaded_files):
        ...
        # --- 1. Inicializa uma linha temporária de dados ---
        temp_metrics_row = {'Simulacao': [], 'CasoFalta': [], 'Local_m1': [], 'Cor': []}
        ...
        try:
            ...
            # Preenche a linha temporária (APENAS se o arquivo for lido)
            temp_metrics_row['Simulacao'].append(simName)
            temp_metrics_row['CasoFalta'].append(caseName)
            temp_metrics_row['Cor'].append(currentColor)

            # Processa m1
            try: m1_loc = data['m1_location'][0, 0]; 
            except: m1_loc = 0;
            temp_metrics_row['Local_m1'].append(m1_loc)
            
            if 'ts' not in data: raise ValueError("Estrutura inválida: falta o struct 'ts'")
            struct_ts = data['ts'][0, 0]
            
            # --- 2. Processamento de Pontos e Cálculo de Métricas ---
            for p in PONTOS_BASE:
                for var in ['I', 'V']:
                    vn = f"{var}_{p}"
                    ...
                    try:
                        ...
                        for idx, fase in enumerate(FASES):
                            ...
                            # Armazenamento (Se o cálculo falhar, retorna NaN)
                            metrics_data[f'{var}{p}_Pico_Fase{fase}'].append(np.max(np.abs(y_fase)) if y_fase.size > 0 else np.nan)
                            metrics_data[f'{var}{p}_THD_Fase{fase}'].append(h_metrics['THD'] if f.size > 0 else np.nan)
                            metrics_data[f'{var}{p}_H3_Fase{fase}'].append(h_metrics['H3'] if f.size > 0 else np.nan)
                            metrics_data[f'{var}{p}_H5_Fase{fase}'].append(h_metrics['H5'] if f.size > 0 else np.nan)
                            metrics_data[f'{var}{p}_H7_Fase{fase}'].append(h_metrics['H7'] if f.size > 0 else np.nan)
                        ...
                        # Se houver qualquer falha no dado deste ponto, preenche com NaN
                        for fase in FASES:
                            for m in METRICAS_TYPES:
                                metrics_data[f'{var}{p}_{m}_Fase{fase}'].append(np.nan)
        except Exception as e:
            st.error(f"Erro fatal ao ler {matFile}: {e}")
            # Se o arquivo for ilegível, remove as entradas de identificação
            # para evitar o ValueError no Pandas.
            metrics_data['Simulacao'].pop()
            metrics_data['CasoFalta'].pop()
            metrics_data['Cor'].pop()
            metrics_data['Local_m1'].pop()
            continue
    ...
    # ... (O restante do código de barra e interface é mantido) ...
    if not df.empty:
        ...
        if spectrum_data:
            ...
            sp_fig.update_layout(title='Espectro Harmônico (I_T2F)', yaxis_title='Amplitude (A)', barmode='group')
